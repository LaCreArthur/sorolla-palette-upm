...
**Pre-build validation**: Failing checks are logged into the build log as LAUNCH BLOCKER warnings automatically via `IPreprocessBuildWithReport`. They never stop the build - early builds must be able to reach the stores before every vendor console entry can exist - but a build with launch-blocker warnings is not launch-ready. (Warnings, not errors: Unity fails a build that logs an error during it.)
